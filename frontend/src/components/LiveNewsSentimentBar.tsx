...
export const LiveNewsSentimentBar: React.FC<LiveNewsSentimentBarProps> = ({ symbol }) => {
  ...
  if (isLoading || !newsData) {
    return (
      <div className="glass-panel-3d rounded-2xl p-5 text-center">
        <div className="w-5 h-5 border-2 border-brand-500 border-t-transparent rounded-full animate-spin mx-auto mb-2" />
        <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
          Analyzing real-time news sentiment & loss risk probability for {symbol}...
        </p>
      </div>
    );
  }
    ...
    <div className="glass-panel-3d rounded-2xl p-4 sm:p-6 space-y-5 transition-all duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/80 dark:border-slate-800/80 pb-4">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-amber-50 dark:bg-amber-950/80 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-amber-50 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
        ...
        <div
          className={`flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold border self-start sm:self-auto ${
            isLowRisk
              ? "bg-profit-50 dark:bg-profit-950/80 text-profit-700 dark:text-profit-300 border-profit-200 dark:border-profit-800"
              : isModerateRisk
              ? "bg-indigo-50 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800"
              : "bg-risk-50 dark:bg-risk-950/80 text-risk-700 dark:text-risk-300 border-risk-200 dark:border-risk-800"
          }`}
        >
          {isLowRisk ? (
            <ShieldCheck className="w-4 h-4 text-profit-600 dark:text-profit-400" />
          ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div
              className={`absolute top-0 bottom-0 rounded-full transition-all duration-500 ${
                isLowRisk
                  ...
                  : isModerateRisk
                  ? "bg-gradient-to-r from-indigo-400 to-indigo-600 shadow-glow-cyan"
                  : "bg-gradient-to-r from-rose-400 to-rose-600 shadow-glow-rose"
              }`}
        ...
        <div className="p-4 rounded-xl bg-brand-50/60 dark:bg-brand-950/40 border border-brand-100 dark:border-brand-900/60 space-y-2.5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-brand-700 dark:text-brand-300 flex items-center gap-1">
          ...
          <div className="pt-1 flex items-center gap-2 text-[11px] text-brand-700 dark:text-brand-300 font-semibold">
      ...
      <div className="space-y-2.5">
        ...
        <div className="space-y-2">
          {(showAllNews ? newsData.headlines : newsData.headlines.slice(0, 3)).map((item, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/60 hover:bg-white dark:hover:bg-slate-850 transition-all space-y-1 glass-card-hover"
            >
              <div className="flex items-start justify-between gap-3">
                <a
                  ...
                  className="font-bold text-xs text-slate-900 dark:text-white hover:text-brand-600 dark:hover:text-brand-400 flex items-center gap-1 group"
                >
                ...
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase shrink-0 ${
                    item.impact.includes("Bullish") || item.impact.includes("Positive")
                      ? "bg-profit-100 dark:bg-profit-950 text-profit-800 dark:text-profit-300 border border-profit-200 dark:border-profit-800"
              ...
              <div className="flex items-center gap-2 text-[10px] text-muted dark:text-slate-500 font-medium pt-0.5">
