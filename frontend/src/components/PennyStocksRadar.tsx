...
export const PennyStocksRadar: React.FC<PennyStocksRadarProps> = ({ onSelectStock }) => {
  ...
  return (
    <div className="glass-panel-3d rounded-2xl p-5 space-y-4 transition-all duration-300">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-base font-extrabold text-slate-900 dark:text-white tracking-tight">
                Vetted Small-Cap & Penny Opportunities (&lt; ₹150)
              </h2>
            ...
            <p className="text-xs text-muted dark:text-muted-dark">
              Institutional filter: Promoter pledge &le; 5% + Positive Operating Cash Flow + High NSE Delivery accumulation
            </p>
          </div>
        </div>
      ...
      {/* Grid of Vetted Small Caps */}
      {isLoading ? (
        <div className="py-8 text-center space-y-2">
          <div className="w-6 h-6 border-2 border-amber-500 border-t-transparent rounded-full animate-spin mx-auto" />
          <p className="text-xs text-slate-500 dark:text-muted-dark font-medium">Screening Indian small-cap universe for verified turnaround catalysts...</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="px-1.5 py-0.2 text-[9px] font-extrabold rounded bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
